...
namespace SdtdMultiServerKit.WebApi.Middlewares
{
    ...
    public class SteamReturnMiddleware : OwinMiddleware
    {
        ...
        {
            // Check whether request path is /api/auth/steam/return
            if (context.Request.Path == new PathString("/api/auth/steam/return"))
            {
                try
                {
                    // Handle Steam Callbacklogic
                    var queryParams = context.Request.Query;

                    // 1. Get Steam Return parameter（ `openid.claimed_id` Etc）
                    ...
                    // 2. Validate parameters meet expectations（Additional validation logic can be added here）
                    ...
                    return; // Ensure the next middleware is not invoked
                }
                catch (Exception ex)
                {
            ...
            // Ifnot /api/auth/steam/return，Usebelowmiddleware
        ...
        private static string ExtractSteamId(string claimedId)
        {
            ...
            // Check URL Whether Steam  OpenID Before
